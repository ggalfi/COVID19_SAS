...
proc sql;
	create table covid.mcmc_weekly_input as 
		...
		where week>=valid_from and week<=valid_to and  rank_region_by_cases<=5
			and (not outlier) and (not  new_deceased =.)
			 /* and region_name='New York' and country='US'*/
		group by country, region_code, region_name
		order by country, region_code, region_name, week;
quit;
data _null_;
	init=0.002;	
...
title "MCMC analysis of weekly regional data, R0 stepwise (RK4 solver)";
proc mcmc
	...
	monitor=(mortality i0 R0_0 R0_1 R0_split beta gamma)
	plots(smooth)=all;
	by country region_code region_name;
...
ods html path="&coviddir/results/mcmc_weekly" (URL=NONE) file="mcmc_quad.html";
title "MCMC analysis of weekly regional data, R0 quadratic (RK4 solver)";
proc mcmc
	data=covid.mcmc_weekly_input
	diag=all
	seed=20200802 nmc=1000000
	nbi=50000
	ntu=50000
	thin= 500
	propcov=ind
	mchistory= detailed
	outpost=covid.mcmc_weekly_quad_post
	monitor=(mortality i0 R0_0 R0_half R0_1  /*log_R0_lin log_R0_quad*/ gamma)
	plots(smooth)=all;
	by country region_code region_name;
	
	preddist outpred=covid.mcmc_weekly_quad_pred  saveparm;
	ods output
		predsumint=covid.mcmc_weekly_quad_predsum
		postsumint=covid.mcmc_weekly_quad_postsumint;
	parms
		log_mort &mort_mu
		log_i0 &i0_mu
		log_R0_0 &R0_mu
	...
	prior log_R0_0 ~ normal(&R0_mu, sd=&R0_var);
	prior log_R0_1 ~ normal(&R0_mu, sd=&R0_var);
	prior log_R0_half ~ normal(&R0_mu, sd=&R0_var);
	prior log_gamma ~ normal(&gamma_mu, sd=&gamma_var);
	beginnodata;
		array arrpdec[52];
		mortality=exp(log_mort);
